  ...
  async function fetchResumo() {
    ...
    try {
      ...
      try {
        ...
        const resComissoes = await fetch(`/api/v1/lideranca/comissoes?mesReferencia=${mesReferencia}`);
        if (resComissoes.ok) {
          const comissoesData = await resComissoes.json();
    ...
    <div className="font-sans space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Visão geral da sua equipe de {resumo?.totalConsultoresPf || 0} consultores PF
        </p>
      </div>
          ...
          <div className="space-y-2">
            {resumo && resumo.totalConsultoresPf > 0 ? (
              ...
              <Link
                ...
                className="block p-3 border rounded-lg hover:bg-gray-50"
              >
                <p className="font-medium text-sm">Criar Primeiro Consultor PF</p>
                <p className="text-xs text-gray-500">
