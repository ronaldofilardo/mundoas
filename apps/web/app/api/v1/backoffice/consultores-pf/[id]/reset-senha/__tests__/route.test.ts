...
describe("POST /api/v1/backoffice/consultores-pf/[id]/reset-senha", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("deve rejeitar se o usuário não for backoffice", async () => {
    ...
    const res = await POST({} as any, { params: { id: "cp-1" } });
    expect(res.status).toBe(403);
  });

  it("deve retornar 404 se o consultor não for encontrado", async () => {
    (requireBackofficeWithScope as any).mockResolvedValue({
      session: { user: { id: "bo-user-1" } },
      backofficeId: "bo-1",
      error: null,
    });
    (prisma.consultorPf.findFirst as any).mockResolvedValue(null);

    const res = await POST({} as any, { params: { id: "cp-inexistente" } });
  ...
  it("deve resetar senha, marcar senhaTemporaria e retornar link", async () => {
    ...
    (prisma.consultorPf.findFirst as any).mockResolvedValue({
      id: "cp-1",
      nome: "Consultor Maria",
      usuario: {
        id: "u-2",
        email: "maria@consultora.com",
        senhaHash: "$2a$12$samplehashconsultora",
        status: "ATIVO",
      },
    });
