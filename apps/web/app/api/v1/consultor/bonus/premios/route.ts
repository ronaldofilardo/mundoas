...
export async function GET() {
  ...
  const [saldo, premios, backoffice] = await Promise.all([
    ...
    prisma.backoffice.findUnique({
      where: { id: backofficeId! },
      select: { catalogoUrl: true },
    }),
  ]);

  return ok({ ciclo: { id: ciclo.id, nome: ciclo.nome, status: ciclo.status }, saldo, premios, catalogoUrl: backoffice?.catalogoUrl ?? null });
}
