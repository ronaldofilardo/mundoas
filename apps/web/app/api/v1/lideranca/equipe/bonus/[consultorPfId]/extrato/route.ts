...
export async function GET(
  ...
  if (inicio || fim) {
    where.criadoEm = {};
    if (inicio) {
      (where.criadoEm as Record<string, unknown>).gte = new Date(
        `${inicio}T00:00:00`,
      );
    }
    if (fim) {
      (where.criadoEm as Record<string, unknown>).lte = new Date(
        `${fim}T23:59:59`,
      );
    }
  }

  const [movimentacoes, somaCreditos, somaDebitos, somaEstornos] =
    await Promise.all([
  ...
  return ok({
    ...
    movimentacoes: movimentacoes.map((m) => ({
      ...
      ciclo: m.cicloPontos?.nome ?? "Ciclo",
      criadoEm: m.criadoEm,
    })),
  });
}
