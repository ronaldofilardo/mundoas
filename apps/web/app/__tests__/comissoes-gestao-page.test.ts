...
describe("Comissões Gestão - Página e Funcionalidades", () => {
  ...
  afterAll(async () => {
    await prisma.comissaoEquipe.deleteMany({
      where: { equipe: { usuario: { email: { endsWith: "@asa.test" } } } },
    });
    ...
    await prisma.procedimentoPF.deleteMany({
      where: {
        OR: [
          ...
          { gestor: { usuario: { email: { endsWith: "@asa.test" } } } },
        ],
      },
    });
    ...
    await prisma.equipe.deleteMany({
      where: { tipo: "COMERCIAL", usuario: { email: { endsWith: "@asa.test" } } },
    });
    await prisma.equipe.deleteMany({
      where: { tipo: "LIDERANCA", usuario: { email: { endsWith: "@asa.test" } } },
    });
    await prisma.procedimentoPF.deleteMany({
      where: {
        OR: [
