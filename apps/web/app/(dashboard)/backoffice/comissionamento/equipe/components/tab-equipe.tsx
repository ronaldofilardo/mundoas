...
import { useEquipe, type EquipeItem } from "../hooks/use-equipe";
...
export function TabEquipe() {
  ...
  function handleEditarComercial(item: EquipeItem) {
    ...
    setComercialEditando({
      ...
      telefone: "",
      funcao: item.funcao,
      tipoLideranca: item.tipoLideranca ? (item.tipoLideranca as "GESTOR" | "COMERCIAL") : undefined,
      tipo: undefined,
      status: item.status,
                      ...
                      <td className="p-3 border-t">
                        {item.kind === "comercial" ? (
                          ...
                          <div>
                            <p className="text-xs text-gray-800 font-medium">
                              Liderança:{" "}
                              {item.tipo === "COMERCIAL" ? "Comercial" : "Gestor"}
                            </p>
                            {(item.consultorPfs?.length ?? 0) > 0 && (
                              <p className="text-xs text-gray-500">
